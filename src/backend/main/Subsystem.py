...
class Subsystem:
    ...
    def connect_to(self, ss, conn_args=(socket.AF_INET, socket.SOCK_DGRAM)):
        ...
        print(f"{self.identifier} ({self.ip_and_port}) connected to {ss.identifier} ({ss.ip_and_port}) using '{s.getsockname()[0]}'")

    def disconnect_from(self, ss_identifier):
        s = self.sockets[ss_identifier]
        print(f"{self.identifier} ({self.ip_and_port}) disconnected from {ss_identifier}")
        s.close()
